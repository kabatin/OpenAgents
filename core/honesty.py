...
# 語尾はペルソナによって変わるので、丁寧形だけでなく口語の言い切り
# （「〜したっス」等）も並べておく。取りこぼすと検出が素通りするため。
...
CLAIMS = {
    ...
    # 納期追跡の会話操作。実行手段が無いまま「キャンセルします」と引き受ける
    # 口約束が起きるので、完了形だけでなく約束形も
    # 検出する（マーカーが実行されていれば -# 行＝証拠が必ず付く）。
    # 「納期/追跡」を必須にして、リマインダー等の別機能の取消と混同しない。
    # 疑問形（〜しますか？）は主張ではないので除外
    "action": re.compile(r"(?:納期|追跡|期日)[^\n]{0,15}?"
                         ...
                         + _DONE + ")"),
    # 記憶・認識の更新。人間の訂正に「認識を更新します」と答えたのに何も
    # 保存されない、が実際に起きた。受け皿（事実台帳）ができたので、
    # 約束形も検出して口約束を封じる。疑問形は主張ではないので除外
...
def build_failed_claim_note(kinds):
    ...
    return (f"⚠️ 失敗しました: {labels}はできていません"
            "（理由は下の ⚠️ 行です）。直してからもう一度"
            "依頼してください🙇")
...
def build_fake_done_note(missing):
    ...
    return (f"⚠️ 失敗しました: {labels}は実行できていません。"
            "お手数ですがもう一度依頼してください🙇")
