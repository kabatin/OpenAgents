...
背景（実運用で踏んだ穴）: 人間が「いまの状況はこうなっている」と訂正し、
エージェントが「認識を更新します」と答えたのに、DBには何も残らなかった。
原因は指針（rules）・決定（decisions）・語（terms）のどれでもない
「事実の現状」に受け皿が無かったこと。書く場所が無いので、誠実に
更新しようとしても口約束で終わる構造だった。
...
def build_skill_note(recent_facts=None):
    ...
    return (
        ...
        "- 例: [FACT: 備品の発注 | 初回分は納品済み。追加分は来月入荷予定]\n"
        "取り消しを頼まれたら: [FACT_CANCEL: id]\n"
        f"{listing}"
        "使い分け: **今後の振る舞い**の指示は [RULE:]、**決まったこと**は"
        "台帳が自動記録、**言葉の表記**は [TERM:]、"
        "**いまどうなっているか**はこの [FACT:]。\n"
        "**書かないもの**: 追跡タスクの期日・担当・完了状態、リマインダーの"
        "日時、シート監視の設定——これらは専用の項目があるので、そちらの"
        "マーカー（[ACTION_DUE:] 等）で直す。FACTに書くと二重管理になり、"
        "予定が動いたときに古い記述が残って混乱を生む。\n"
