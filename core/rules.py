...
# 訂正の一次フィルタ（最終判断はLLM＝二重ゲート）
CORRECTION_RE = re.compile(
    r"違う|ちがう|じゃなくて|ではなく|間違|まちが|誤り|誤解|正しくは|訂正|"
    r"そうじゃな")

# 訂正語を使わない「事実の言い直し」も拾う。
# 例: 「発注自体は滞りなく完了していて、追加分は来月入荷の予定です」
# 訂正語ゼロ・リプライでもないメンションだったため従来は二重に取りこぼした。
STATE_RE = re.compile(
    r"(?:完了|終わ[っり]|済[んみ]|中止|見送|延期|変更|変わ[っり]|"
    r"になりました|になっています|してあります|してます|してない|"
    r"予定です|ことになった)")
...
def looks_like_state_update(text):
    """訂正語なしの「状況説明・事実の言い直し」に見えるか（純粋関数）。
    質問文は除外（「完了しましたか？」を事実申告と誤解しないため）。"""
    t = (text or "").strip()
    if not t or _QUESTION_RE.search(t):
        return False
    return bool(STATE_RE.search(t))


def build_correction_note(state_only=False, tools=False):
    """訂正・状況説明を検知したときの学習指示（RM#17＋2026-08-18の事実台帳）。
    state_only=True は訂正語なしの状況説明（確度が低いので言い回しを弱める）。
    訂正由来のルールは「訂正: 」で始める＝週次レポートで集計できる。
    tools=True はツールループ本番: マーカーではなくツール名で指示し、
    自分の癖への指摘は save_lesson で教訓に残す（学習の行動接続）。"""
    head = ("【訂正の学習】この発言はあなたの直前の発言への訂正・指摘の"
            "可能性が高い。\n" if not state_only else
            "【状況の反映】この発言は、状況や事実を教えてくれている"
            "可能性がある（訂正とは限らない）。\n")
    if tools:
        return (
            head +
            "(1) **事実の現状**を教えられた・訂正されたと判断したら、"
            "save_fact(topic, fact) で必ず記録する。「認識を更新します」と"
            "書くだけで済ませない（ツールを呼ばなければ何も残らない）。\n"
            "(2) **今後の振る舞い**を直された場合は save_rule で保存する"
            "（本文は「訂正: 」で始める）。\n"
            "(3) **自分の癖・やり方への指摘**（「ログを読めていない」「勝手に"
            "断定しないで」等）は save_lesson(text, polarity=\"down\") で"
            "教訓として残す。反論せず、まず引き直してから答える。\n"
            "(4) 訂正でも状況共有でもないなら普通に返信し、何も保存しない。\n"
            "(5) 謝るときの作法: 一言の謝罪＋原因＋実際に打った手（ツールで"
            "実行したこと）を短く。言い訳を長くしない。")
    return (
        head +
        "(1) **事実の現状**を教えられた・訂正されたと判断したら、"
        "[FACT: 主題 | 事実の内容] で必ず記録する"
        "（例: [FACT: 備品の発注 | 初回分は納品済み。追加分は"
        "来月入荷予定]）。"
        "「認識を更新します」と書くだけで済ませてはいけない"
        "（保存されず口約束になるため。実際にマーカーを出すこと）。\n"
        "(2) **今後の振る舞い**を直された場合は [RULE: ...] で保存する。"
        "ルール本文は「訂正: 」で始めること"
        "（例: [RULE: channel | 訂正: 定例の開始は19:30ではなく19:00]）。\n"
        "(3) 訂正でも状況共有でもない（別の話題・単なる意見・進行中の相談）なら"
        "普通に返信し、何も保存しない。訂正・共有されていない内容まで"
        "勝手に保存しないこと。\n"
        "(4) 謝るときの作法: 一言の謝罪＋なぜ間違えたか（原因）＋再発防止に"
        "打った手（実際にマーカーで実行したこと）をセットで短く伝える。"
        "言い訳を長くしない。")
