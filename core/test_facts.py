...
class ParseTest(unittest.TestCase):
    def test_parse_and_extract(self):
        text, adds, cancels, errors = facts.extract_markers(
            "了解っス\n[FACT: 備品の発注 | 初回分は納品済み。追加分は"
            "来月入荷予定]\n[FACT_CANCEL: 7]")
        self.assertEqual(text, "了解っス")
        self.assertEqual(adds[0]["topic"], "備品の発注")
        self.assertIn("納品済み", adds[0]["fact"])
    ...
    def _add(self, topic, fact, mid=None):
        with db.connect(self.db_path) as conn:
            return db.add_fact(
                ...
                channel_id=7 if mid else None, stated_by="人A",
                created_at="2026-08-18T05:52")

    def test_same_topic_supersedes(self):
        self._add("備品の発注", "来月上旬に入荷予定で納品は未実施")
        fid, superseded = self._add("備品の発注", "初回分は納品済み")
        self.assertEqual(superseded, 1)      # 古い認識が上書きされる
        with db.connect(self.db_path) as conn:
            rows = db.search_facts(conn, ["備品"])
        self.assertEqual(len(rows), 1)       # activeは最新だけ
        self.assertEqual(rows[0]["id"], fid)
        self.assertIn("納品済み", rows[0]["fact"])

    def test_other_topic_untouched(self):
        self._add("納期", "来月到着")
        _fid, superseded = self._add("価格", "1点3000円")
        self.assertEqual(superseded, 0)
        with db.connect(self.db_path) as conn:
            self.assertEqual(db.count_facts(conn), 2)

    def test_ledger_block_has_source_link(self):
        self._add("備品の発注", "初回分は納品済み", mid=1539149855996256338)
        block = facts.build_ledger_block(self.db_path, ["備品"], "999")
        self.assertIn("事実台帳", block)
        self.assertIn("[備品の発注] 初回分は納品済み", block)
        self.assertIn("人Aさん談", block)
...
class CorrectionDetectionTest(unittest.TestCase):
    ...
    def test_question_not_treated_as_statement(self):
        self.assertFalse(rules.looks_like_state_update("納品は完了しましたか？"))
...
class FakeDoneTest(unittest.TestCase):
    ...
    語尾がわざと口語なのは意図的。ペルソナ次第で語尾は変わるので、
    丁寧形しか拾えない検出になっていないことをここで担保する。
    """

    def test_memory_claim_without_deed_is_caught(self):
        answer = ("了解っス、初回分は納品済み、追加分は来月入荷という"
                  "理解で認識更新するっス。")
    ...
    def test_plain_answer_not_flagged(self):
        self.assertEqual(honesty.detect_fake_done("初回分の納品は完了っスね"), [])
